...
def evaluate_sqls_parallel(db_files, questions, pred_sqls, ground_truth_sqls, num_cpus=1, timeout=200):
...
def execute_sqls_parallel(db_files, sqls, num_cpus=1, timeout=200):
    pool = mp.Pool(processes=num_cpus)
...
def mark_invalid_sqls(db_files, sqls):
    ...
    execution_results = []
    execute_sqls_parallel(db_files, sqls, num_cpus=20, timeout=200)
...
def major_voting(db_files, pred_sqls, sampling_num, return_random_one_when_all_errors=True):
    ...
    execute_sqls_parallel(db_files, pred_sqls, num_cpus=20, timeout=200)
    execution_results = sorted(execution_results, key=lambda x:x['data_idx'])
...
def run_eval(gold_file, pred_file, db_path, mode, save_pred_sqls=False, num_cpus=20, timeout=200):
    ...
    if mode == "greedy":
        ...
        if save_pred_sqls:
            with open(pred_file[:-5] + "_pred_greedy_sqls.json", "w", encoding="utf-8") as f:
        ...
        if save_pred_sqls:
            with open(pred_file[:-5] + "_pred_majority_sqls.json", "w", encoding="utf-8") as f:
        ...
        raise ValueError("mode should be in [greedy, majority, pass@k]")

if __name__ == "__main__":
    opt = parse_option()
    run_eval(opt.gold, opt.pred, opt.db_path, opt.mode)