...
def execute_sqls_parallel(all_db_files, all_sqls, all_execution_results, num_cpus=10, timeout=200):
...
def evaluate(mode, gold_result_dir, eval_standard, gold_file, pred_file, db_path, save_pred_sqls=False):
    ...
    execute_sqls_parallel(all_db_files, all_pred_sqls, all_execution_results, num_cpus=40, timeout=200)
    all_execution_results = sorted(all_execution_results, key=lambda x: x["data_idx"])

    pred_sqls = []
    for idx in tqdm(range(len(gold))):
        execution_results = all_execution_results[idx*sampling_num: (idx+1)*sampling_num]
        if mode == "greedy":
            # For greedy calculation, pred_sqls is a list of SQL query strings.
            ...
            # For majority calculation, pred_sqls is a list of SQL query strings.
    ...
    if mode in ["greedy", "majority"]:
        ...
        if save_pred_sqls:
            suffix = "-pred-greedy-sqls.json" if mode == "greedy" else "-pred-major-voting-sqls.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gold_result_dir', type = str, default = "")
    parser.add_argument('--eval_standard', type = str, default = "")
