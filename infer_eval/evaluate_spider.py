...
def run_spider_eval(gold_file, pred_file, db_path, ts_db_path, mode, save_pred_sqls=False):
    assert mode in ["greedy", "majority"]
    ...
    cmd = f'python3 -u path_to/test_suite_sql_eval/evaluation.py --gold {gold_file} --pred {temp_file_name} --db {db_path} --etype exec'
    print(cmd)
    ...
    if ts_db_path != "":
        ...
        cmd = f'python3 -u path_to/test_suite_sql_eval/evaluation.py --gold {gold_file} --pred {temp_file_name} --db {ts_db_path} --etype exec'
        result = subprocess.run(cmd, shell=True, text=True, capture_output=True)
        stdout = result.stdout
        print(result.stderr)
        # match the last num in the string
...
if __name__ == "__main__":
    opt = parse_option()
    run_spider_eval(opt.gold, opt.pred, opt.db_path, opt.ts_db_path, opt.mode)