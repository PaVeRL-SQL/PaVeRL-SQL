...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            num_constant_steps = int(optim_config.get("lr_constant_steps", -1))
            ...
            if num_constant_steps < 0:
                num_constant_steps_ratio = optim_config.get("lr_constant_steps_ratio", 0.0)
                num_constant_steps = int(num_constant_steps_ratio * total_steps)

            print(f"Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}, num_constant_steps: {num_constant_steps}")

            if warmup_style == "constant":
                actor_lr_scheduler = get_constant_schedule_with_warmup(optimizer=actor_optimizer, num_warmup_steps=num_warmup_steps)
            elif warmup_style == "cosine":
                actor_lr_scheduler = get_cosine_schedule_with_warmup(optimizer=actor_optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=total_steps, min_lr_ratio=min_lr_ratio, num_cycles=num_cycles, num_constant_steps=num_constant_steps)
            else:
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        num_constant_steps = int(config.optim.get("lr_constant_steps", -1))
        warmup_style = config.optim.get("warmup_style", "constant")
        if num_warmup_steps < 0:
            num_warmup_steps_ratio = config.optim.get("lr_warmup_steps_ratio", 0.0)
            num_warmup_steps = int(num_warmup_steps_ratio * total_steps)
        if num_constant_steps < 0:
            num_constant_steps_ratio = config.optim.get("lr_constant_steps_ratio", 0.0)
            num_constant_steps = int(num_constant_steps_ratio * total_steps)
        ...
        if warmup_style == "constant":
            ...
            critic_lr_scheduler = get_cosine_schedule_with_warmup(optimizer=critic_optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=total_steps, num_constant_steps=num_constant_steps)
        else:
            raise NotImplementedError(f"Warmup style {warmup_style} is not supported")
